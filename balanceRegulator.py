...
class BalanceRegulator():
    ...
    def __init__(self, imu):
        ...
        self.pid = PID(0.012, 0.0002, 0.007, 0.0003)                         # PID控制器
    ...
    def estimateSpeed(self):
        ...
        """
        self.filtered_estimated_speed = 0.5 * self.filtered_estimated_speed + 0.5 * ((self.L_rps_vel + self.R_rps_vel) / 2.0)*20
    ...
    def regulateLoop(self):
        ...
        # print('time tick:{}'.format(dt))
        self.prev_time = now
        mpu_angle = self.filter.complementary(self.imu)
        self.current_angle = mpu_angle - ANGLE_OFFSET                         # 加上偏置的角度，求得当前的实际偏离的角度        #current_angle = self.previous_angle*0.5 + current_angle*0.5           # 减去偏置的角度，求得当前的实际偏离的角度
